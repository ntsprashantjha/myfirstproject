...
/**
 * Servlet implementation class DischargeToSeaStopDeckWashing
 */
@WebServlet("/DischargeToSeaStopDeckWashing")
public class DischargeToSeaStopDeckWashing extends HttpServlet {
	 ...
	 */
	public DischargeToSeaStopDeckWashing() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("**json data from discharge to sea stop deck washing controller**" + jObj);
		if (new SoftwareLicenceTime().remainTime() < 0) {
			...
			if (ActiveUserStatus.userstatus(GetUserMacId.getClientMACAddress(request)) == 0) {
				...
				try {
					...
					com.nts.grb2.model.DischargeToSeaStopDeckWashing dischtoseastpdkwsh = new com.nts.grb2.model.DischargeToSeaStopDeckWashing();
					...
					dischtoseastpdkwsh.setRemark(jObj.optString("remark2Form").toUpperCase());
					...
					lsEntryUpdt.strt_deck_wash(jObj.getString("date"), jObj.getString("time"),
							timezone, "stop_deck_washing_grb2", 0, category_type, entry_filled_dt[0]);
