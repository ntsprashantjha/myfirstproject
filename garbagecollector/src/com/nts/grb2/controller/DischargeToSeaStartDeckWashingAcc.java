...
/**
 * Servlet implementation class DischargeToSeaStartDeckWashingAcc
 */
@WebServlet("/DischargeToSeaStartDeckWashingAcc")
public class DischargeToSeaStartDeckWashingAcc extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(DischargeToSeaStartDeckWashingAcc.class);


	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public DischargeToSeaStartDeckWashingAcc() {
		super();
		...
		if (new SoftwareLicenceTime().remainTime() < 0) {
			...
			if (ActiveUserStatus.userstatus(GetUserMacId.getClientMACAddress(request)) == 0) {
				...
				try {
					...
					com.nts.grb2.model.DischargeToSeaStartDeckWashing_Acc dischtoseasrtdkwsh = new com.nts.grb2.model.DischargeToSeaStartDeckWashing_Acc();
					...
					System.out.println("**before last entry updateion**");
					...
					lsEntryUpdt.strt_deck_wash(jObj.getString("date"), jObj.getString("time"), timezone,
							"start_deck_washing_acc_grb2", 1, jObj.getString("garbageCategoryForm"),
							entry_filled_dt[0]);
					out.print(ReturnResponse.success_condition());
					System.out.println("before sleep condtion");
					Thread.sleep(4000);
