...
public class GetQuantityForSludgeReport {

	public static JSONObject getTankData(int tnkid, List<String> dates, Connection con)
	...
	public static JSONArray getQuantity(String startdate) throws SQLException, JSONException {
		...
		PreparedStatement ps = con
				.prepareStatement("select DISTINCT tank_id from tanks_entry_detalis_orb WHERE tank_type_id <> 3");
		ResultSet rs = ps.executeQuery();
		...
		for (int i = 0; i < tankid.size(); i++) {
			...
			alldata.put(getTankData(tankid.get(i), getPreviousDates(startdate), con));
		}
