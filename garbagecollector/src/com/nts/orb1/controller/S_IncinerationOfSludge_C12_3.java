...
/**
 * Servlet implementation class S_IncinerationOfSludge_C12_3
 */
@WebServlet("/S_IncinerationOfSludge_C12_3")
public class S_IncinerationOfSludge_C12_3 extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(S_IncinerationOfSludge_C12_3.class);

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public S_IncinerationOfSludge_C12_3() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("**json data from Sludge Incineration controller**" + jObj);
		...
		if (new SoftwareLicenceTime().remainTime() < 0) {
			...
			if (ActiveUserStatus.userstatus(GetUserMacId.getClientMACAddress(request)) == 0) {
				...
				try {
					...
					com.nts.orb1.model.SludgeIncineration12_3Orb1 obj = new com.nts.orb1.model.SludgeIncineration12_3Orb1();
						...
						if (new LastEntryCondCheck().lastEntryCheck(jObj.getString("stop_date"), " ") > 0) {
							...
							if (new LastEntryCondCheck().lastEntryDailyTankSoundsheet(stopdate[0], " ") <= 0) {
								...
								if (CheckBoilerCapacity.checkCapacity(strtdate, stpdate,
										jObj.getDouble("qty_incinerated"))) {

									System.out.println("Start Date " + jObj.getString("stop_date"));
									System.out.println("Start Time " + jObj.getString("start_time"));
									...
									System.out.println("Stop Date : - " + stpdate);
									System.out.println("Start Date : -" + strtdate);

									obj.setSource_tank(jObj.getString("source_tank"));
									obj.setQuantity_incinerated(jObj.getDouble("qty_incinerated"));
									...
									if (AutomaticCheck.isAutomatic(jObj.getString("source_tank"))) {
										new AutomaticEntryGeneration().automaticEntryByAutoTypeTankSou(mac,
												...
												jObj.getDouble("qty_retained"), jObj.getDouble("qty_incinerated"),
												userid, entry_id);
									}
									...
									mstrObj.lastEntryUpdation("Incineration of sludge C 12.3",
									...
									CreateAudit.auditUpdate(userid, "Incineration of sludge C 12.3",
											"Entry Created In Sludge Incineration C 12.3", " ", " ", mac, "ORB1");
