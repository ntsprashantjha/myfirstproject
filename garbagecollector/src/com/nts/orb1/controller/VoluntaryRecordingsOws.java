...
@WebServlet("/VoluntaryRecordingsOws")
public class VoluntaryRecordingsOws extends HttpServlet {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(VoluntaryRecordingsOws.class);

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public VoluntaryRecordingsOws() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("**json data from Voluntry Recording-OWS overboard Line Inspection controller**" + jObj);
		...
		if (new SoftwareLicenceTime().remainTime() < 0) {
			...
			if (ActiveUserStatus.userstatus(GetUserMacId.getClientMACAddress(request)) == 0) {
				...
				try {
					...
					com.nts.orb1.model.VoluntaryRecordingsOwsOrb1 obj = new com.nts.orb1.model.VoluntaryRecordingsOwsOrb1();
					...
					if (checkPreviousDateData.checkDate(stopdate[0])) {
						...
						if (new LastEntryCondCheck().lastEntryCheck(stopdate[0], " ") > 0) {
							...
							if (new LastEntryCondCheck().lastEntryDailyTankSoundsheet(stopdate[0], " ") <= 0) {
								...
								obj.setFindings(jObj.getString("findings"));
								...
								if (missed) {
									...
									obj2.setMissed_entry_type("Voluntry Recording-OWS overboard Line Inspection");
									...
									mstrObj.lastEntryUpdation("Voluntry Recording-OWS overboard Line Inspection",
								...
								CreateAudit.auditUpdate(userid, "VOLUNTARY RECORDINGS",
										"Voluntry Recording-OWS overboard Line Inspection", " ", " ", mac, "ORB1");
