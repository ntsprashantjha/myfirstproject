...
import com.nts.orb1.model.BilgeTransferBwTwoTanksD13Orb1;
 ...
 */
@WebServlet("/B_TransferBtTwoBligeTanks_D13")
public class B_TransferBtTwoBligeTanks_D13 extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(B_TransferBtTwoBligeTanks_D13.class);

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public B_TransferBtTwoBligeTanks_D13() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("**json data from Transfer Between Bilge Tanks controller**" + jObj);
		...
		if (new SoftwareLicenceTime().remainTime() < 0) {
			...
			if (ActiveUserStatus.userstatus(GetUserMacId.getClientMACAddress(request)) == 0) {
				...
				try {
					...
					BilgeTransferBwTwoTanksD13Orb1 obj = new BilgeTransferBwTwoTanksD13Orb1();
					...
					if (checkPreviousDateData.checkDate(stopdate[0])) {
						...
						if (new LastEntryCondCheck().lastEntryCheck(stopdate[0], " ") > 0) {
							...
							if (new LastEntryCondCheck().lastEntryDailyTankSoundsheet(stopdate[0], " ") <= 0) {
								...
								mstrObj.lastEntryUpdation("Transfer Between Bilge Tanks D13",
								...
								CreateAudit.auditUpdate(userid, "Bilge Transfer",
										"Transfer Between Bilge Tanks", " ", " ", mac, "ORB1");
