...
import com.nts.orb1.model.BreakingOfMarpolAnnex1RelatedValueOrEquipmentEntity;
...
/**
 * Servlet implementation class BreakingOfMarpolAnnex1RelatedValueOrEquipment
 */
@WebServlet("/BreakingOfMarpolAnnex1RelatedValueOrEquipment")
public class BreakingOfMarpolAnnex1RelatedValueOrEquipment extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(BreakingOfMarpolAnnex1RelatedValueOrEquipment.class);

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public BreakingOfMarpolAnnex1RelatedValueOrEquipment() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("**json data from BreakingOfMarpolAnnex1RelatedValueOrEquipmentEntity**" + jObj);
		...
		if (new SoftwareLicenceTime().remainTime() < 0) {
			...
			if (ActiveUserStatus.userstatus(GetUserMacId.getClientMACAddress(request)) == 0) {
				...
				try {
					...
					BreakingOfMarpolAnnex1RelatedValueOrEquipmentEntity obj = new BreakingOfMarpolAnnex1RelatedValueOrEquipmentEntity();
					...
					if (checkPreviousDateData.checkDate(stopdate[0])) {
						...
						if (new LastEntryCondCheck().lastEntryCheck(stopdate[0], " ") > 0) {
							...
							if (new LastEntryCondCheck().lastEntryDailyTankSoundsheet(stopdate[0], " ") <= 0) {
								...
								obj.setStop_date(jObj.getString("stop_date"));
								obj.setStart_date(jObj.getString("stop_date"));

								obj.setValveNumber(jObj.getString("seal_number"));

								obj.setSealNumber(jObj.getString("valve_no"));
								...
								if (missed) {
									...
									obj2.setMissed_entry_type(
											"Breaking Of Marpol Annex1 Related Value Or Equipment Entity");
								...
								mstrObj.lastEntryUpdation("Breaking Of Marpol Annex1 Related Value Or Equipment Entity",
								...
								CreateAudit.auditUpdate(userid, "Bilge Transfer",
										"Breaking Of Marpol Annex1 Related Value Or Equipment Entity", " ", " ", mac,
										"ORB1");
