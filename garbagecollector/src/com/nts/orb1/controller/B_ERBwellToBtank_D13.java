...
import com.nts.orb1.model.BligeERBwellToBtankD13Orb1;
...
@WebServlet("/B_ERBwellToBtank_D13")
public class B_ERBwellToBtank_D13 extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(B_ERBwellToBtank_D13.class);

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public B_ERBwellToBtank_D13() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("**json data from BlidgeERBwellToBtankD13Orb1 controller**" + jObj);
		...
		if (new SoftwareLicenceTime().remainTime() < 0) {
			...
			if (ActiveUserStatus.userstatus(GetUserMacId.getClientMACAddress(request)) == 0) {
				...
				try {
					...
					BligeERBwellToBtankD13Orb1 obj = new BligeERBwellToBtankD13Orb1();
						...
						if (new LastEntryCondCheck().lastEntryCheck(stopdate[0], " ") > 0) {
							...
							if (AutomaticCheck.isAutomatic(jObj.getString("destination_tank"))) {
								...
								new AutomaticEntryGeneration().automaticEntryByAutoTypeTankDes(mac, jObj.getString("stop_date"),
										jObj.getString("destination_tank"),
							...
							mstrObj.lastEntryUpdation("Engine Room Bilge Wells To Bilge Tank D13", jObj.getString("stop_date"), "",
							...
							CreateAudit.auditUpdate(userid, "Engine Room Bilge Wells To Bilge Tank D13",
									"Entry Created In Engine Room Bilge Wells To Bilge Tank D13", " ", " ", mac, "ORB1");
