...
import com.nts.orb1.model.FailureOilFltrEquipmentF19_20_21Orb1;
 ...
 */
@WebServlet("/FailureOfOilFilterEquipment_F19_20_21")
public class FailureOfOilFilterEquipment_F19_20_21 extends HttpServlet {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(FailureOfOilFilterEquipment_F19_20_21.class);

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public FailureOfOilFilterEquipment_F19_20_21() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("**json data from Failure Oil Filter controller**" + jObj);
		...
		if (new SoftwareLicenceTime().remainTime() < 0) {
			...
			if (ActiveUserStatus.userstatus(GetUserMacId.getClientMACAddress(request)) == 0) {
				...
				try {
					...
					com.nts.orb1.model.FailureOilFltrEquipmentF19_20_21Orb1 obj = new FailureOilFltrEquipmentF19_20_21Orb1();
					obj.setOfficer_id(
					...
					if (checkPreviousDateData.checkDate(stopdate[0])) {
						...
						if (new LastEntryCondCheck().lastEntryCheck(stopdate[0], " ") > 0) {
							...
							if (new LastEntryCondCheck().lastEntryDailyTankSoundsheet(stopdate[0], " ") <= 0) {
								...
								obj.setReason_of_failure(jObj.getString("reason_of_failure"));
								obj.setIs_equipment_functional(jObj.optString("is_equipment_functional"));

								if (jObj.optString("is_equipment_functional").equalsIgnoreCase("yes")) {

									obj.setTime_of_operation(jObj.getString("time_of_operation"));
									
								} else {
									obj.setReason_for_not_operational(jObj.optString("reason_for_not_operational"));
									obj.setSeal_no_for_valve(jObj.optString("seal_no_for_valve"));
									obj.setValve_no(jObj.optDouble("valve_no"));
								}
								...
								if (missed) {
									...
									obj2.setMissed_entry_type("FAILURE OF OIL FILTERING EQUIPMENT (F 19,20,21)");
									...
									mstrObj.lastEntryUpdation("FAILURE OF OIL FILTERING EQUIPMENT (F 19,20,21)",
								...
								CreateAudit.auditUpdate(userid, "Oil Filtering",
										"FAILURE OF OIL FILTERING EQUIPMENT (F 19,20,21)", " ", " ", mac, "ORB1");
