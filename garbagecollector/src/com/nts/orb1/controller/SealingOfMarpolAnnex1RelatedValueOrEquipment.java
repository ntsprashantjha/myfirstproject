...
import com.nts.orb1.model.SealingOfMarpolAnnex1RelatedValueOrEquipmentEntity;
...
/**
 * Servlet implementation class SealingOfMarpolAnnex1RelatedValueOrEquipment
 */
@WebServlet("/SealingOfMarpolAnnex1RelatedValueOrEquipment")
public class SealingOfMarpolAnnex1RelatedValueOrEquipment extends HttpServlet {
	 ...
	 */
	public SealingOfMarpolAnnex1RelatedValueOrEquipment() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("**json data from SealingOfMarpolAnnex1RelatedValueOrEquipment frontend**" + jObj);
		...
		if (new SoftwareLicenceTime().remainTime() < 0) {
			...
			if (ActiveUserStatus.userstatus(GetUserMacId.getClientMACAddress(request)) == 0) {
				...
				try {
					...
					SealingOfMarpolAnnex1RelatedValueOrEquipmentEntity obj = new SealingOfMarpolAnnex1RelatedValueOrEquipmentEntity();
					...
					if (checkPreviousDateData.checkDate(stopdate[0])) {
						...
						if (new LastEntryCondCheck().lastEntryCheck(stopdate[0], " ") > 0) {
							...
							if (new LastEntryCondCheck().lastEntryDailyTankSoundsheet(stopdate[0], " ") <= 0) {
								...
								obj.setStopDate(jObj.getString("stop_date"));
								...
								if (missed) {
									...
									obj2.setMissed_entry_type("SealingOfMarpolAnnex1RelatedValueOrEquipment");
									...
									mstrObj.lastEntryUpdation("SealingOfMarpolAnnex1RelatedValueOrEquipment",
								...
								CreateAudit.auditUpdate(userid, "Bilge Transfer",
										"SealingOfMarpolAnnex1RelatedValueOrEquipment", " ", " ", mac, "ORB1");
